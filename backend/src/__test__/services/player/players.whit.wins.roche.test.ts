...
describe("getAllPlayersWithWinPercentage", () => {
  beforeEach(() => {
    ...
    (GameRepository.findGamesByPlayerId as jest.Mock).mockImplementation(playerId => {
      switch (playerId) {
        case "1":
  ...
  it("debe devolver jugadores con sus porcentajes de victorias", async () => {
    const playersWithWinPercentage = await playerServices.getAllPlayersWithWinPercentage();
