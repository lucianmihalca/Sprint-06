...
class PlayerController {
  ...
  public async createPlayer(req: Request, res: Response): Promise<void> {
    try {
      ...
      const newPlayer = await playerRepository.createPlayer(playerName);

      // Responder con los detalles del jugador creado
      res.status(200).json({ playerName: newPlayer.name, playerId: newPlayer.id });
  ...
  public async updatePlayerName(req: Request, res: Response): Promise<void> {
    try {
      ...
      const existingPlayer = await playerRepository.findPlayerByName(playerName);
      if (existingPlayer && existingPlayer.id.toString() !== id) {
        res.status(400).json({ message: "El nombre ya está en uso por otro jugador" });
        return;
      }

      // Buscar y actualizar el nombre del jugador
      const updatedPlayer = await playerRepository.updatePlayerName(id, playerName);
      if (!updatedPlayer) {
